...
# 2) Les dossiers contenant 'LAB2' vont dans le train/val
train_val_paths = [p for p in all_paths if 'LAB2' in p]

# 3) Le reste (pas de 'LAB2') ira dans le test
test_paths = [p for p in all_paths if 'LAB2' not in p]

print(f"Nombre total de dossiers : {len(all_paths)}")
print(f"  -> train+val   : {len(train_val_paths)} (contient 'LAB2')")
print(f"  -> test_paths  : {len(test_paths)} (n'a pas 'LAB2')")

# 4) Sépare les LAB2 en train et val (ex. 20% en validation)
...
# 5) Sauvegarde des chemins (optionnel)
save_dir = '/store/wizar/HAR_code_complex/radardataM/radar_detection1/wRadar/Polar/stratifyv2_GTnew_test_LAB1'
...
# 6) Construit les Datasets puis DataLoaders
...
model_save_path = os.path.join(save_dir, 'modelpolar_stratifyv2_GTnew_trainLAB1.pth')
...
loss_curve_path = os.path.join(save_dir, 'loss_curve_stratifyv2_GTnew_trainLAB1.png')
